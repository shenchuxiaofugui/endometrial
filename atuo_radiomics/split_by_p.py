...
def main_run(excel_path, output_path='', repeat_times=50):
    ...
    if output_train_df is not None and output_path != '':
        output_train_df.to_csv(os.path.join(output_path, 'train_clinical_feature.csv'), index=False, encoding="utf_8_sig")
        output_test_df.to_csv(os.path.join(output_path, 'test_clinical_feature.csv'), index=False, encoding="utf_8_sig")
